...
def dsrl_config(
    ...
    **kwargs,
    ...
    def make_noise_actor(observation_shape: Tuple[int, ...], action_dim: int) -> nn.Module:
        # Tanh-squashed Gaussian noise policy. The tanh bound is critical: it
        # (1) keeps the input to the BC flow inside [-noise_scale, noise_scale],
        #     well within the noise_scale * N(0, I) support the flow was trained on,
        # (2) makes target_entropy = -action_dim a reachable SAC fixed point
        #     (unbounded Gaussian has natural entropy > -target_entropy, which
        #     causes log_alpha to monotonically collapse to 0 and the noise
        #     actor's pre-tanh mean to drift arbitrarily).
        return Policy(
            ac_dim=action_dim,
            ob_dim=int(np.prod(observation_shape)),
            discrete=False,
